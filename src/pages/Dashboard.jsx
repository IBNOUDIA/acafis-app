...
export default function Dashboard() {
  ...
  // Statut membres
  const membresAJour    = members.filter(m => m.financial?.status === 'à_jour').length;
  const membresRetard   = members.filter(m => m.financial?.status === 'retard_majeur' || m.financial?.status === 'retard_mineur').length;
          ...
          <>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '1.5rem' }}>
                <div>
                  ...
                  <p style={{ margin: '0.3rem 0 0', color: 'rgba(255,255,255,0.7)', fontSize: '0.85rem' }}>
                    1er août 2026 à 14h00 — Mode Hybride
                  </p>
                </div>
            ...
            <div style={{
              ...
              {[
                ...
                { label: 'Total Collecté',   value: `${(stats?.totalCollecte || 0).toLocaleString('fr-FR')} FCFA`, icon: '💰', color: '#c9973a' },
              ].map((stat, i) => (
                <div key={i} style={{
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1.6fr 1fr', gap: '1.5rem', marginBottom: '1.5rem' }}>
              ...
              <div style={{ background: '#fff', borderRadius: '12px', padding: '1.5rem', boxShadow: '0 2px 12px rgba(0,0,0,0.06)' }}>
                <h3 style={{ color: '#1a3a6b', margin: '0 0 1.25rem', fontSize: '0.95rem', fontWeight: 700 }}>
                  📊 Cotisations reçues par mois (2026)
                </h3>
                      ...
                      <div key={i} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '4px' }}>
                        ...
                        <div style={{ width: '100%', display: 'flex', alignItems: 'flex-end', height: '100px' }}>
                          <div style={{
                            ...
                            background: graph.data[i] > 0
                              ? 'linear-gradient(180deg, #c9973a, #e8b86d)'
                              : '#ede9e0',
                ...
                <div style={{ textAlign: 'center', marginBottom: '1rem' }}>
                  <div style={{ position: 'relative', display: 'inline-block' }}>
                    ...
                    <div style={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%,-50%)', textAlign: 'center' }}>
                      ...
                      <div style={{ fontSize: '0.6rem', color: '#8a8a8a' }}>membres</div>
                    </div>
                  </div>
                </div>

                {[
                  { label: 'Présents AG',    value: membresPresents, color: '#2d6a4f', icon: '✅' },
                  { label: 'Procurations',   value: 10,              color: '#c9973a', icon: '📋' },
                  { label: 'Absents',        value: 11,              color: '#c0392b', icon: '❌' },
                  { label: 'Statut inconnu', value: 7,               color: '#8a8a8a', icon: '⬜' },
                ].map((item, i) => (
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.5rem', marginBottom: '1.5rem' }}>
              ...
              <div style={{ background: '#fff', borderRadius: '12px', padding: '1.5rem', boxShadow: '0 2px 12px rgba(0,0,0,0.06)' }}>
                ...
                {nextMeeting ? (
                  <div>
                    <div style={{
                      background: '#1a3a6b', borderRadius: '10px', padding: '1rem',
                      ...
                      <div style={{ fontSize: '0.82rem', color: 'rgba(255,255,255,0.7)' }}>
                        🕐 {nextMeeting.time} · {nextMeeting.duration} min
                      </div>
                      <div style={{ fontSize: '0.82rem', color: 'rgba(255,255,255,0.7)' }}>
                    ...
                    <div style={{ fontSize: '0.78rem', color: '#8a8a8a', textAlign: 'center' }}>
                      📋 {nextMeeting.agenda?.length || 0} points à l'ordre du jour
                    </div>
                  </div>
                ) : (
              ...
              <div style={{ background: '#fff', borderRadius: '12px', padding: '1.5rem', boxShadow: '0 2px 12px rgba(0,0,0,0.06)' }}>
                <h3 style={{ color: '#1a3a6b', margin: '0 0 1rem', fontSize: '0.95rem', fontWeight: 700 }}>
                  🏦 Compte BHS — Solde actuel
                </h3>
                <div style={{ textAlign: 'center', padding: '1rem 0' }}>
                  <div style={{ fontSize: '2rem', fontWeight: 800, color: '#1a3a6b' }}>
                    3 817 410
                  </div>
                  <div style={{ fontSize: '0.88rem', color: '#c9973a', fontWeight: 700 }}>FCFA</div>
                  <div style={{ fontSize: '0.72rem', color: '#8a8a8a', marginTop: '0.25rem' }}>
                    Au 13/07/2026
                  </div>
                </div>
                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem', marginTop: '0.75rem' }}>
                  {[
                    { label: 'Solde initial 01/01/2026', value: '6 909 110 FCFA', color: '#1a3a6b' },
                    { label: 'Cotisations reçues',       value: '+ 920 200 FCFA', color: '#2d6a4f' },
                    { label: 'Dépenses architecture',    value: '- 4 011 900 FCFA', color: '#c0392b' },
                  ].map((item, i) => (
            ...
            <div style={{ marginBottom: '1rem' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(140px, 1fr))', gap: '0.85rem' }}>
                {[
                  ...
                  { icon: '💰', label: 'Finance',        sublabel: 'BHS',           path: '/payments',       color: '#c9973a' },
